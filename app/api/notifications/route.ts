...
type NotificationRow = {
  ...
  actorName: string | null; taskId: string | number | null; is_read: boolean; createdAt: string;
};
...
export async function GET(request: Request) {
  ...
  await ensureTable();
  const rows = await sql`SELECT id, recipient_user_id, kind, title, body, actor_name AS "actorName", task_id AS "taskId", is_read, created_at AS "createdAt"
